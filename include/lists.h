...
class tlist
{
	...
	template<typename T> T *ToArray2(cloneFunction *mapcopyfunc) {
		tnode *n;
		T *a = (T*)malloc( sizeof(T) * (count+1) );
		T *p=a;

		for( n = nodes; n; n = n->next ) {
			*p = (T)mapcopyfunc(n->data);
			p++;
		}
		*p = (T)NULL;
	...
	template<typename T> T Reduce(T joinfunc(T init, void*next)) {
		...
		for( n = nodes; n; n = n->next ) {
			a=joinfunc((void*)a,n->data);
		}

		return a;
	};
	template<typename T> T Reduce2(reduceFunction *joinfunc) { // T joinfunc(T init,void*next)) {
		tnode *n;
		T a = joinfunc(a,NULL);

		for( n = nodes; n; n = n->next ) {
			a=(T)joinfunc((void*)a,n->data);
		}
